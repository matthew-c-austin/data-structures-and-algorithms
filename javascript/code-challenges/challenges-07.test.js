...
// Renaming the parameter 'arr' to 'str' since that makes more sense
...
const listFoods = (recipe) => {
  let result = [];
  /* Since we're not allowed to use split, we are going to assume (probably unwisely) that any recipe fed into this function has the format of the object in the example as well as two spaces in the description of ingredients before the food itself is described. Also, we'll assume that there's only a single space between words.

  We'll use indexOf twice to find it then use slice to get the ingredient. */

  recipe.ingredients.forEach(ingredient => {
    let firstIdx = ingredient.indexOf(' ');
    let secondIdx = ingredient.indexOf(' ', firstIdx + 1);
    let food = ingredient.slice(secondIdx + 1);
    result.push(food);
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  // The first word is the verb always, i.e., the first index of a split string array.
...
const removeEvenValues = (arr) => {
  // This solution only resizes the array once, which is faster than any splice/slice/split or filter nonsense. We have two increment variables and test the condition to filter, reassigning values as we go through the array. At the end we set the length to the increment j and return the resized array.
  let j = 0;

  arr.forEach((e, i) => {
    if (e % 2 === 1) {
      // This extra conditional is here because we don't really need to reassign values if the indices are equivalent
      if (i!==j) arr[j] = e;
      j++;
    }
  });
...
const removeLastCharacters = (str, numberOfCharacters) => {
  /* Might be a more elegant way to do it, but we'll just use slice and three conditionals

  It's not explicit what needs to be return with a value of 0 for numberOfCharacters...and it's not one of the tests. If this were a interview I'd ask for clarification here, but I'll just assume if you want no characters trimmed then you'd want the whole string. */
...
const totalSumCSV = (str) => {
  // Oh my God we get to use reduce if we want! What a cool method...
  const arr = str.split(',');
...
const removeVowels = (str) => {
  // We can use regex here. We'll assume sometimes 'y' is not a thing here.
...
const extractVowels = (str) => {
  /* There are a few components here. We can first split the string into an array using ECMA2015+ [...str].

  From S.0 [https://stackoverflow.com/questions/30912663/sort-a-string-alphabetically-using-a-function].
  ...
  The spread notation fixes that issue. Next we can use regex .test to push each character into one of two new arrays without and with vowels.

  Finally, we can use localeCompare on the vowels array to sort them, followed by join for both arrays. Sort is a O(NlogN) task, whereas the spread operator and joins are all O(n), for an overall time complexity of O(NlogN). Space complexity is O(N), since we have two arrays that total to be the number of characters in the string. */

  const strArr = [...str];
  const extractedVowelsArr = [];
  const vowelsArr = [];
  const regex = /[aeiou]/i;
  strArr.forEach(char => {
    if (regex.test(char)) {
      ...
      extractedVowelsArr.push(char);
    }
  });

  const extractedVowelsStr = extractedVowelsArr.join('');
  const sortedVowelsStr = vowelsArr.sort((a, b) => a.localeCompare(b)).join('');

  return [extractedVowelsStr,sortedVowelsStr];
};
