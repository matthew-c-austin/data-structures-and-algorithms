...
const sortByChildren = (charArray) => {
  // Looks like the data set has been modified from previous challenges to always have a children property. We can just compare lengths
...
const citiesAtoJ = (arr) => {
  // Since this is an array of cities, and there are cities with multiple words, we'll just look for A-J in the first letter of each element with .filter and .test
...
let hangman = (str) => {
  return str.replaceAll(/[aeiou]/gi, '_');
};
...
// Really don't understand the purpose of limiting the vertical pipe in this case. You would use it. Because what I have below will return true for seasells and eashells, but I can't figure out a way to not have that be the case without | alternatives.

const findShells = (str) => {
  const regex = /\bs?(eas)?(h)?(ells)\b/gi;
  const result = str.match(regex);
  return result === null ? [] : result;
};
