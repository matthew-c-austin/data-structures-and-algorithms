...
const longestString = (arr) => {
  // Love having to dig into the tests to see how to handle corner cases. And of course this brings up things like well what if the indices aren't strings? Do we care to account for that?
  if (!arr.length) return -1;
  let currentLongestStringIdx = 0;
  for (let i = 0; i < arr.length; i++) {
    currentLongestStringIdx = arr[i].length > arr[currentLongestStringIdx].length ? i : currentLongestStringIdx;
  }
  return currentLongestStringIdx;
};
...
const standardizePhoneNumbers = (arr) => {
  // We can iterate over each string and do a regex test, then push a finished string into a mapped array

  return arr.map(string => {
    let standardizedPhoneNumber = '';
    for (let char of string) {
      if (/\d/.test(char)) {
        standardizedPhoneNumber = standardizedPhoneNumber.concat(char);
      }
    }
...
const onlyOddChars = (str) => {
  ...
  for (let i in str) {
    if (i % 2 === 1) {
      oddChars = oddChars.concat(str[i]);
    }
  }
  return oddChars;
};
...
const unenrollBrook = (arr) => {
  return arr.map(roster => {
    return roster.filter(student => !student.includes('Brook'));
  });
};
...
const sortByDay = (arr) => {
  // Only way to do this with the array above is to iterate through the arr once for every day of the week for O(n^2)
