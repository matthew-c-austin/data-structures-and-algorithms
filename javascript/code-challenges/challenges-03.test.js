...
const { map, filterArray } = require("cheerio/lib/api/traversing");
...
const notInFirstArray = (forbiddenValues, arr) => {
  /* We could use forbiddenValues.includes(value) in our filter method, but this is inefficient overall. That's an O(n) time complexity operation (at worst) inside of an O(m) operation for O(mn), where n is the length of 'forbiddenValues' and m is the length of 'arr'.

  Instead, we'll cycle through forbiddenValues once to create a Set with the forbidden values, and use that in the .filter method for an O(1) operation. This makes it an O(m+n) problem.

  Note: this turns the space complexity from O(1) to O(n) */

  // Create set with forbidden values
  const forbiddenValuesSet = new Set();
  for (let value of forbiddenValues) {
    forbiddenValuesSet.add(value);
  }

  // Filter arr based on forbidden values (pass only values that are not in the forbidden values set)
  return arr.filter(value => !(forbiddenValuesSet.has(value)));
};
...
const getStatName = (arr, minBaseStat) => {
  const filteredArr = getBaseStatGreaterThan(arr, minBaseStat);
  return filteredArr.map(object => object.stat.name);
  // The .filter() method will return the entire element based on the criteria, I'm not entirely sure how to combine the operations short of just looping through the entire array and conditionally pushing the object property into a new one without the .filter method.
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(character => !('children' in character));
};
