...
const sortPeople = (arr) => {
  //We won't use localeCompare for this one just for fun
...
const sortPeopleBetter = (arr) => {
  // Nasty nested conditionals blech
  return arr.sort((a,b) => {
...
const sortMeetingsByDay = (arr) => {
  // Create sorter, assuming the problem statement holds and no weekend days are used. It is made lowercase and then case-insensitively checked later, in case the Meeting object definition has an uncapitalized input.
  ...
  return arr.sort((a, b) => {
    const firstDayNumber = dayOfWeekOrder[a.dayOfWeek.toLowerCase()];
    const secondDayNumber = dayOfWeekOrder[b.dayOfWeek.toLowerCase()];
...
const sortSchedule = (arr) => {
  ...
  return arr.sort((a, b) => {
    const firstDayNumber = dayOfWeekOrder[a.dayOfWeek.toLowerCase()];
    const secondDayNumber = dayOfWeekOrder[b.dayOfWeek.toLowerCase()];
    const firstStartNumber = Number(a.start);
    const secondStartNumber = Number(b.start);
    const firstEndNumber = Number(a.end);
    const secondEndNumber = Number(b.end);

    if (firstDayNumber - secondDayNumber > 0) {
      return 1;
    } else if (firstDayNumber - secondDayNumber < 0) {
      return -1;
    } else {
      // Uf two meetings are on the same day, sort the meetings in the order that they start.
      if (firstStartNumber - secondStartNumber > 0) {
        return 1;
      } else if (firstStartNumber - secondStartNumber < 0) {
        return -1;
      } else {
        // If two meetings start at the same time on the same day, the shorter meeting should come first.
        return firstEndNumber - secondEndNumber;
      }
    }
  });
};
